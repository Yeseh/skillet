...
use crate::{PanSource, lex::{Lexer, Token, TokenKind}};
...
impl<'a> PanParse<'a> {
    ...
    fn make_ref_node(&mut self, token_iter: &mut Peekable<Iter<'_, Token>>, start_offset: u32, kind: RefKind) {
    ...
    pub fn parse(&mut self) {
        ...
        while let Some(t) = token_iter.next() {
            match t.kind {
                // Emit body node
                TokenKind::BodyText => {
                    self.nodes.push(Node::Body { source_range: t.range.clone() });
                }
                TokenKind::Tick => {
                    ...
                    match peek {
                        Some(p) => {
                            match p.kind {
                                TokenKind::RefSkill => self.make_ref_node(&mut token_iter, t.range.start, RefKind::Skill),
                                TokenKind::RefReference => self.make_ref_node(&mut token_iter, t.range.start, RefKind::Reference),
                                TokenKind::RefAgent => self.make_ref_node(&mut token_iter, t.range.start, RefKind::Agent),
                                TokenKind::RefCmd => self.make_ref_node(&mut token_iter, t.range.start, RefKind::Cmd),
                                TokenKind::RefPath => self.make_ref_node(&mut token_iter, t.range.start, RefKind::Path),
                                TokenKind::RefUrl => self.make_ref_node(&mut token_iter, t.range.start, RefKind::Url),
                                TokenKind::BodyText => {
                        ...
                        None => self.errors.push(ParseError { range: t.range.clone(), kind: ParseErrorKind::UnexpectedEOF })
                    }
                },
...
mod tests {
    ...
    fn parse_str(src: &str) -> ParseResult {
        ...
        parsed.parse();
        ParseResult { nodes: parsed.nodes, errors: parsed.errors }
    }
    ...
    fn fragment_with_surrounding_body() {
        ...
        assert!(result.nodes.iter().any(|n| matches!(n, Node::Fragment { value, .. } if value == "my-frag")));
    }

    #[test]
    fn unclosed_fragment_produces_error() {
        let result = parse_str("{>my-fragment");
        assert!(!result.errors.is_empty(), "expected an error for unclosed fragment");
        assert!(result.errors.iter().any(|e| matches!(e.kind, ParseErrorKind::UnclosedFragment)));
    }
    ...
    fn fragment_open_without_id_produces_error() {
        ...
        assert!(result.errors.iter().any(|e| {
            matches!(e.kind, ParseErrorKind::ExpectedRefValueAfterFragmentOpen | ParseErrorKind::UnclosedFragment)
        }));
    }
    ...
    fn escaped_body_produces_escaped_body_node() {
        ...
        assert!(result.errors.is_empty(), "expected no errors");
        assert!(result.nodes.iter().any(|n| matches!(n, Node::EscapedBody { .. })));
    }
    ...
    fn escaped_body_source_range_covers_both_delimiters() {
        ...
        let result = parse_str(src);
        assert!(result.errors.is_empty());
        let escaped = result.nodes.iter().find(|n| matches!(n, Node::EscapedBody { .. }));
    ...
    fn unclosed_escaped_body_produces_error() {
        ...
        assert!(!result.errors.is_empty());
        assert!(result.errors.iter().any(|e| matches!(e.kind, ParseErrorKind::UnclosedEscape)));
    }
    ...
    fn tick_wrapped_unknown_text_produces_ref_suspect() {
        ...
        assert!(result.errors.is_empty(), "expected no errors");
        assert!(result.nodes.iter().any(|n| matches!(n, Node::RefSuspect { .. })));
    }
    ...
    fn ref_suspect_source_range_covers_both_ticks() {
        ...
        let result = parse_str(src);
        assert!(result.errors.is_empty());
        let node = result.nodes.iter().find(|n| matches!(n, Node::RefSuspect { .. }))
    ...
    fn tick_followed_by_body_without_closing_tick_is_body_node() {
        ...
        assert!(result.errors.is_empty(), "expected no errors for unclosed markdown inline code");
    ...
    fn lone_trailing_tick_produces_unexpected_eof_error() {
        let result = parse_str("`");
        assert!(!result.errors.is_empty());
        assert!(result.errors.iter().any(|e| matches!(e.kind, ParseErrorKind::UnexpectedEOF)));
    }
    ...
    fn skill_ref_produces_ref_node_with_skill_kind() {
        ...
        assert!(result.errors.is_empty(), "expected no errors, got: {}", result.errors.len());
        assert_eq!(result.nodes.len(), 1);
        match &result.nodes[0] {
            Node::Ref { kind: RefKind::Skill, value, .. } => assert_eq!(value, "my-skill"),
    ...
    fn agent_ref_produces_ref_node_with_agent_kind() {
        ...
        assert_eq!(result.nodes.len(), 1);
        match &result.nodes[0] {
            Node::Ref { kind: RefKind::Agent, value, .. } => assert_eq!(value, "my-agent"),
    ...
    fn cmd_ref_produces_ref_node_with_cmd_kind() {
        ...
        assert!(result.errors.is_empty(), "expected no errors");
        match &result.nodes[0] {
            Node::Ref { kind: RefKind::Cmd, value, .. } => assert_eq!(value, "git-status"),
    ...
    fn path_ref_produces_ref_node_with_path_kind() {
        ...
        assert!(result.errors.is_empty(), "expected no errors");
        match &result.nodes[0] {
            Node::Ref { kind: RefKind::Path, value, .. } => assert_eq!(value, "src/main.rs"),
    ...
    fn url_ref_produces_ref_node_with_url_kind() {
        ...
        assert!(result.errors.is_empty(), "expected no errors");
        match &result.nodes[0] {
            Node::Ref { kind: RefKind::Url, value, .. } => assert_eq!(value, "https://example.com"),
    ...
    fn reference_ref_produces_ref_node_with_reference_kind() {
        ...
        assert!(result.errors.is_empty(), "expected no errors");
        match &result.nodes[0] {
            Node::Ref { kind: RefKind::Reference, value, .. } => assert_eq!(value, "some-ref"),
    ...
    fn unclosed_ref_produces_error() {
        let result = parse_str("`skill::my-skill");
        assert!(!result.errors.is_empty());
        assert!(result.errors.iter().any(|e| matches!(e.kind, ParseErrorKind::UnclosedRef)));
    }
    ...
    fn mixed_document_produces_correct_node_count() {
        ...
        assert!(result.errors.is_empty(), "expected no errors");
        assert!(result.nodes.iter().any(|n| matches!(n, Node::Ref { kind: RefKind::Skill, .. })));
        assert!(result.nodes.iter().any(|n| matches!(n, Node::Fragment { .. })));
        assert!(result.nodes.iter().any(|n| matches!(n, Node::Body { .. })));
    }
    ...
    fn multiple_refs_of_different_kinds() {
        ...
        assert!(result.errors.is_empty());
        assert!(result.nodes.iter().any(|n| matches!(n, Node::Ref { kind: RefKind::Skill, .. })));
        assert!(result.nodes.iter().any(|n| matches!(n, Node::Ref { kind: RefKind::Agent, .. })));
    }
}